...
- Qwen3-Coder 30B was rerun from the same starting commit after manual Ollama launch with `OLLAMA_CONTEXT_LENGTH=65536`; `ollama ps` confirmed live 65,536 context. It completed in 77.3 s but deleted allowed `main.js`, created forbidden `main.tsx`, skipped the requested build, and rendered a duplicate `#root`. Increasing context alone did not improve reliability. The manual server was stopped; no `ollama.service` exists to persist the setting.
