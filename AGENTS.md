...
- **Remote-agent delivery parity:** A delegated remote implementation agent receives the same complete packet text, exact writable paths, existing-harness instruction, observable assertions, required gates, commit requirement, and correction limit as the local Qwen agent. A retry never receives an abbreviated substitute prompt. Nested delegation is disabled unless every nested implementation worker receives that same complete packet and is constrained to the same writable paths. A delegated remote implementation agent never creates placeholder files, installs dependencies, or changes package configuration; any such return is rejected before review.
