...
- Foundry owns element type, roles, required relationships, state semantics, reserved `data-control`, `data-part`, `data-size`, and presence-only state hooks. Public logical state hooks are `data-disabled`, `data-readonly`, `data-invalid`, `data-loading`, `data-checked`, `data-indeterminate`, `data-selected`, and `data-open` where applicable. The token and skin contract additionally owns the exact visual value and interaction hooks used by recipes. Foundation attributes are not skin or test contracts.
...
| --- | --- | --- | --- |
| `Button` | `button`; `HTMLButtonElement`; Action button | `children: ReactNode`; `variant?: ButtonVariant` default `primary`; `size?: ControlSize` default `md`; `disabled?: boolean` default false; `loading?: boolean` default false. Icon-only use requires an accessible-name prop. Disabled uses native refusal. Loading remains focusable, sets `aria-disabled` and `aria-busy`, refuses repeat activation, preserves its accessible name, renders an assistive-technology-hidden visual indicator, and announces localized `loading` once on each false-to-true transition through a transient visually hidden polite status sibling with no layout wrapper. | Retain name; remove domain-label categories. |
...
| `NativeSelect` | single semantic `select` inside a private visual root; `HTMLSelectElement`; Select | `children` are one or more `option`/`optgroup` elements; controlled `{ value: string; onChange(event: ChangeEvent<HTMLSelectElement>): void }` or uncontrolled `{ defaultValue?: string; onChange?(event: ChangeEvent<HTMLSelectElement>): void }`; `disabled?`, `required?` default false; `invalid?: boolean | string` default false; `size?: ControlSize` default `md`. Empty string is a permitted consumer-owned value. The private indicator is non-semantic; props, form behavior, focus, and the public ref remain on the select. | Rename current `Select`; reserve `Select` for a styled listbox. |
| `Checkbox` | semantic checkbox `input` inside a private visual root; `HTMLInputElement`; Checkable input | Controlled `{ checked: CheckboxState; onCheckedChange(next: CheckboxState): void }` or uncontrolled `{ defaultChecked?: CheckboxState; onCheckedChange?(next: CheckboxState): void }`, default false; `name?`, `value?: string` default `"on"`, `form?`; `required?`, `disabled?` default false; `invalid?: boolean | string` default false; `size?: ControlSize` default `md`. Mixed sets DOM `indeterminate` and `aria-checked="mixed"`; next user activation requests `true`. Native reset restores the mounted default. The native input retains the hit area, focus, form behavior, semantics, forwarded props, and public ref; its private indicator is presentation only. | Retain; add mixed state, size, and callback requirement. |
| `Switch` | semantic checkbox `input` with `role="switch"` inside a private visual root; `HTMLInputElement`; Checkable input | Controlled `{ checked: boolean; onCheckedChange(next: boolean): void }` or uncontrolled `{ defaultChecked?: boolean; onCheckedChange?(next: boolean): void }`, default false; `name?`, `value?: string` default `"on"`, `form?`; `required?`, `disabled?` default false; `invalid?: boolean | string` default false; `size?: ControlSize` default `md`. Mixed is refused; native reset restores the mounted default. The native input retains the hit area, focus, form behavior, semantics, forwarded props, and public ref; the private track and thumb are presentation only. | Retain; add size and callback requirement. |
| `RadioGroup` | `fieldset` and semantic radio inputs with private presentation indicators; `HTMLFieldSetElement`; Radio group | `name: string`, `label: ReactNode`, `options: RadioOption[]`, `form?`; controlled `{ value: string | null; onValueChange }` or uncontrolled `{ defaultValue?: string | null; onValueChange? }`; `required?`, `disabled?` default false; `invalid?: boolean | string` default false; `size?: ControlSize` default `md`. `RadioOption` is `{ value: string; label: ReactNode; description?: ReactNode; disabled?: boolean }`; at least one unique non-empty value is required. Required may begin empty. Each native input retains its hit area, focus, form behavior, and semantics; the indicator is presentation only. Right/Down select next, Left/Up previous, Home/End first/last, horizontal keys follow direction, and each move requests once. | Retain data API; remove required auto-selection. |
| `SearchField` | private non-semantic visual root containing a search `input` and conditional clear `button`; ref `HTMLInputElement`; Text input applies to the input | Controlled `{ value: string; onValueChange(next: string): void }` or uncontrolled `{ defaultValue?: string; onValueChange?(next: string): void }`; a controlled value may omit `onValueChange` only with `readOnly: true`. Also `name?`, `form?`, `disabled?`, `required?`, `readOnly?` default false; `invalid?: boolean | string` default false; `size?: ControlSize` default `md`; `onClear?(): void`. The root has no semantics or forwarding surface; form behavior, focus, forwarded props, and the public ref remain on the input. Clear requests `""` once, then calls `onClear`; focus is preserved whether accepted or declined. Empty, disabled, or read-only state has no clear button. | Rename `Search`; localize clear and fix callback order. |
