...
| CG-M4-10 | Qwen (remote RunPod) / OpenCode `maestro/Qwen-Code-Assistant` | `apps/lab/src/examples/overlays/popover/PopoverExamples.tsx` | 70–110 | One independent gallery-example file consuming only the accepted public Popover API; no shared control, export, integration, or browser-test ownership. | `npm exec tsc -- --noEmit && npm run check && npm run build && git diff --check` → expected PASS | OpenCode `maestro/Qwen-Code-Assistant` | PASS — remote model available at dispatch on 2026-08-30 | 1 |
