...
For M3–M5, the named local runtime is **OpenCode 1.18.21 / Ollama `qwen3.6:27b`**, supervised by the Codex coordinator. A task's issue checklist line carries that exact assignment for Atlas. The runtime is a routing hypothesis, not permission to dispatch before the packet's preflight and code-share estimate are recorded.
...
- M3–M5 local packets must declare a code-bearing line estimate, exact paths, three to five observable assertions, prerequisite target/dependency state, and one-correction limit before dispatch. The coordinator records forecast-versus-actual local code share at close.
